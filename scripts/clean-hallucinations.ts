...
const inputFile = 'c:/Users/josep/Documents/GitHub/AI_Workspace/Brittney/training-data/futuristic-holoscript.jsonl';
const outputFile = 'c:/Users/josep/Documents/GitHub/AI_Workspace/Brittney/training-data/futuristic-holoscript-clean.jsonl';
...
function cleanHoloScript() {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        try {
            ...
            const keywordsRegex = /^(npc|scene|quest|dialogue|item|ability|achievement|stateMachine|sequence|localizedText|talentTree|skill|recipe|interactive|collectible|networked|spawnPoint|party|chat|pvp|matchmaking|leaderboard|trading|auctionHouse|friends|guild|reconnection|skill|module|object)\b/;
            ...
            .replace(/(\w+):\s*(\([^)]*\)\s*=>\s*{[\s\S]*?})(,?)/gm, (match: string, key: string, func: string, comma: string) => {
            ...
            .replace(/(\w+):\s*(\(\)\s*=>\s*[^,{}\n]+)(,?)/g, (match: string, key: string, func: string, comma: string) => {
            ...
            expressionKeys.forEach(key => {
                ...
                code = code.replace(regex, (match, prefix, val, suffix) => {
                    val = val.trim();
                    if (val === 'true' || val === 'false' || !isNaN(Number(val)) || val.startsWith('"') || val.startsWith('{') || val.startsWith('[')) return match;
