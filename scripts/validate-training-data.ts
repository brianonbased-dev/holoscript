...
async function validateTrainingData() {
    const dataPath = path.join('C:/Users/josep/Documents/GitHub/AI_Workspace/Brittney/training-data/futuristic-holoscript.jsonl');
        ...
        try {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    console.error(`❌ Validation Error at Line ${i + 1}: ${JSON.stringify(result.errors.map((e: any) => e.message))}`);
                    errorCount++;
                }
            } catch (pErr: any) {
