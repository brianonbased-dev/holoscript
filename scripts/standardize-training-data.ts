...
const inputFile = 'c:/Users/josep/Documents/GitHub/AI_Workspace/Brittney/training/brittney-v4-cloud-training-final.jsonl';
const outputFile = 'c:/Users/josep/Documents/GitHub/AI_Workspace/Brittney/training/cloud-training-redeemed.jsonl';
...
function standardizeHoloScript(code: string): string {
    ...
    let s = code
        ...
        .replace(/^\s*(state|stats|logic|visual|audio|physics|ai|events)\s*{/gm, (match) => match.replace('{', ': {'))
        ...
        .replace(/^\s*(?!object|return|if|for|while|switch|catch|spawn|function|action)([\w]+)\s+([^:{\n]+)$/gm, (match, key, val) => {
    ...
    for (let line of lines) {
        ...
        const isBehavior = /^\s*(function|let|const|if|for|while|return|wait|emit|play_sound|spawn_particles|this\.)/.test(line);
        const isBareAssignment = /^\s*[\w_.]+\s*=\s*[^:{]+$/.test(line);
        
        if (inNode && (isBehavior || isBareAssignment) && !trimmed.startsWith('@') && !trimmed.startsWith('logic')) {
        ...
        if (isOpening) {
            const isNode = /^\s*(system|composition|world|object|template|state_machine|state)\b/.test(line);
...
try {
    ...
    const processed = entries.map((l, i) => {
        try {
