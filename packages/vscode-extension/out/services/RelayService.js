...
class RelayService {
    ...
    async handleTransformUpdate(document, id, position, rotation, scale) {
        ...
        await editor.edit(editBuilder => {
            // 1. Update Position
            if (position) {
                this.updateProperty(editBuilder, document, blockStart, blockContent, 'position', `[${position.map(n => parseFloat(n.toFixed(3))).join(', ')}]`);
            }
            // 2. Update Rotation
            if (rotation) {
                ...
                this.updateProperty(editBuilder, document, blockStart, blockContent, 'rotation', `[${rotation.map(n => parseFloat(n.toFixed(3))).join(', ')}]`);
            }
            // 3. Update Scale
            if (scale) {
                this.updateProperty(editBuilder, document, blockStart, blockContent, 'scale', `[${scale.map(n => parseFloat(n.toFixed(3))).join(', ')}]`);
            }
        });
    ...
    updateProperty(editBuilder, document, blockStart, blockContent, propertyName, newValue) {
        ...
        if (propMatch) {
            ...
            const range = new vscode.Range(document.positionAt(propStartAbs), document.positionAt(propEndAbs));
