...
describe('HoloScriptPlusParser - Extended Features', () => {
  ...
  it.skip('Parses @networked trait correctly (needs source parser debugging)', () => {
    const source = `
      cube#networked_box @networked(sync_mode: "reliable", authority: "owner") {
        position: [1, 2, 3]
      }
    `;
  ...
  it.skip('Parses @external_api directive correctly (not yet implemented)', () => {
    const source = `
      object api_sensor @external_api(url: "https://api.iot.com/sensor", method: "GET", interval: 10s) {
  ...
  it.skip('Handles multiple directives and traits (external_api not yet implemented)', () => {
    const source = `
      light#living_room 
        @networked(sync_mode: "state-only")
        @external_api(url: "https://api.home.com/light", interval: 5m)
        @grabbable
      {
        color: "#ffffff"
      }
