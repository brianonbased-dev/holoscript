...
type EstimationMode = 'ambient_intensity' | 'ambient_spherical' | 'directional' | 'environmental_hdr';
...
export const lightEstimationHandler: TraitHandler<LightEstimationConfig> = {
  ...
  onEvent(node, config, context, event) {
    ...
    if (event.type === 'light_estimation_update') {
      ...
      state.intensity = state.intensity * config.smoothing +
        newIntensity * (1 - config.smoothing);
      
      // Color temperature
      if (config.color_temperature && event.colorTemperature !== undefined) {
        const newTemp = event.colorTemperature as number;
        state.colorTemperature = state.colorTemperature * config.smoothing +
          newTemp * (1 - config.smoothing);
