...
type MaterialPreset = 'concrete' | 'wood' | 'glass' | 'metal' | 'fabric' | 'carpet' | 'tile' | 'custom';
...
const MATERIAL_PRESETS: Record<MaterialPreset, Partial<FrequencyBands>> = {
  ...
  wood: { 125: 0.15, 250: 0.11, 500: 0.10, 1000: 0.07, 2000: 0.06, 4000: 0.07 },
  ...
  fabric: { 125: 0.03, 250: 0.09, 500: 0.20, 1000: 0.54, 2000: 0.70, 4000: 0.72 },
  ...
  custom: { 125: 0.10, 250: 0.10, 500: 0.10, 1000: 0.10, 2000: 0.10, 4000: 0.10 },
};
...
export const audioMaterialHandler: TraitHandler<AudioMaterialConfig> = {
  ...
  onAttach(node, config, context) {
    ...
    const absorption = Object.keys(config.absorption_coefficients).length > 0
      ? config.absorption_coefficients
      : MATERIAL_PRESETS[config.material_preset];
    
    // Calculate average absorption
    const values = Object.values(absorption) as number[];
